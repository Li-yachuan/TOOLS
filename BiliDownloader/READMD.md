...
This is a lightweight tool for downloading videos from Bilibili. 

All the source code is contained in `main.py`. To run it, you'll need Python along with the `tkinter` and `yt_dlp` packages. Additionally, please download `ffmpeg.exe` and place it in the same directory as `main.py`.
...
```
pyinstaller -F -w --add-data "ffmpeg.exe;." .\main.py
```
